...
const modalWinnerAnnouncement = document.getElementById(
...
cells.forEach((cell) => {
  ...
  cell.addEventListener(
    "keydown",
    (e) => (e.key === "Enter" || e.key === " ") && handleCellClick(e),
  );
});
...
function getDynamicDelay(level) {
  switch (level) {
    case "easy":
      return Math.random() * 600 + 400; // 0.4s - 1.0s
    case "hard":
      return Math.random() * 1200 + 800; // 0.8s - 2.0s
    case "medium":
    default:
      return Math.random() * 1000 + 600; // 0.6s - 1.6s
  }
}
...
function getEasyMove() {
  ...
  if (Math.random() < 0.2) {
    // 20% chance to find its own winning move
...
function getHardMove() {
  ...
  if (playerCorners.length === 1) {
    ...
    if (board[opposite[playerCorners[0]]] === "")
      return opposite[playerCorners[0]];
  }
